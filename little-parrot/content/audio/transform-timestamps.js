...
const videoName = 'social-000-chloe'

// --- Main execution block ---
async function main() {
  const inputFilePath = `alignment-${videoName}.json`
  const outputFilePath = `output-video-${videoName}.json`
  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      ...
      console.error(
        'Please ensure your input JSON data is saved in a file named `input.json` in the same directory as this script.',
      )
