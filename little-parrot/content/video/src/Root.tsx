import './index.css';

import { Composition, staticFile } from "remotion";
import { LessonVideo, LessonVideoPropsSchema, Captions, LessonVideo2, LessonVideo3, LessonVideo4, LessonVideo5, LessonVideo6, LessonVideo7, LessonVideoLanding, LessonVideoDebugging01, LessonVideoLovableBasics01, LessonVideoLovablePublish01, SocialVideo000, SocialVideo001, SocialVideo002, SocialVideo003, SocialVideo004, SocialVideo005, SocialVideo006, SocialVideo007, SocialVideo008, SocialVideo009, SocialVideo010, SocialVideo011, SocialVideo012, SocialVideo013, SocialVideo014, SocialVideo015, LessonVideoAiAdmin00, SocialVideo016, SocialVideo017 } from "./LessonVideo";
import { FRAME_HEIGHT, FRAME_RATE, FRAME_WIDTH } from './config';

export const RemotionRoot: React.FC = () => {
  return (
    <>
      <Composition
        id="lesson-write-better-with-ai-00-video-00"
        component={LessonVideo2}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 60,
          endDuration: 30,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 60
          const endDuration = 0
          const response = await fetch(staticFile('video-2/text.json'))
          const captions = await response.json()
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />
      <Composition
        id="lesson-write-better-with-ai-00-video-01"
        component={LessonVideo}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 60,
          endDuration: 60,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 60
          const endDuration = 0
          const response = await fetch(staticFile('video-1/text.json'))
          const captions = await response.json()
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />
      <Composition
        id="lesson-write-better-with-ai-00-video-02"
        component={LessonVideo3}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 30,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 0
          const response = await fetch(staticFile('video-3/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />
      <Composition
        id="lesson-write-better-with-ai-00-video-03"
        component={LessonVideo4}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 60,
          endDuration: 30,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 60
          const endDuration = 0
          const response = await fetch(staticFile('video-4/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />
      <Composition
        id="lesson-vibe-coding-tech-00-video-00"
        component={LessonVideo5}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 60,
          endDuration: 30,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 60
          const endDuration = 0
          const response = await fetch(staticFile('video-5/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />
      <Composition
        id="lesson-vibe-coding-tech-00-video-01"
        component={LessonVideo6}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 60,
          endDuration: 30,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 60
          const endDuration = 0
          const response = await fetch(staticFile('video-6/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />
      <Composition
        id="lesson-vibe-coding-product-management-00-video-00"
        component={LessonVideo7}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 60,
          endDuration: 30,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 60
          const endDuration = 0
          const response = await fetch(staticFile('video-7/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="landing-video"
        component={LessonVideoLanding}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 60,
          endDuration: 120,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 60
          const endDuration = 120
          const response = await fetch(staticFile('video-landing/text.json'))
          const captions = await response.json()
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="lesson-vibe-coding-debugging-00-video-00"
        component={LessonVideoDebugging01}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 60,
          endDuration: 10,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 60
          const endDuration = 10
          const response = await fetch(staticFile('video-debugging-01/text.json'))
          const captions = await response.json()
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="lesson-vibe-coding-lovable-basics-00-video-00"
        component={LessonVideoLovableBasics01}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 60,
          endDuration: 10,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 60
          const endDuration = 10
          const response = await fetch(staticFile('video-lovable-basics-01/text.json'))
          const captions = await response.json()
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="lesson-vibe-coding-lovable-publish-00-video-00"
        component={LessonVideoLovablePublish01}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 60,
          endDuration: 10,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 60
          const endDuration = 10
          const response = await fetch(staticFile('video-lovable-publish-01/text.json'))
          const captions = await response.json()
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-000"
        component={SocialVideo000}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 0,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 0
          const response = await fetch(staticFile('social-000/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 0,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-001"
        component={SocialVideo001}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-001/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />
      <Composition
        id="social-002"
        component={SocialVideo002}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-002/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-003"
        component={SocialVideo003}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-003/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-004"
        component={SocialVideo004}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-004/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-005"
        component={SocialVideo005}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-005/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-006"
        component={SocialVideo006}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-006/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-007"
        component={SocialVideo007}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-007/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />


      <Composition
        id="social-008"
        component={SocialVideo008}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-008/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-009"
        component={SocialVideo009}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-009/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />


      <Composition
        id="social-010"
        component={SocialVideo010}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-010/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-011"
        component={SocialVideo011}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-011/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-012"
        component={SocialVideo012}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-012/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-013"
        component={SocialVideo013}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-013/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-014"
        component={SocialVideo014}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-014/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-015"
        component={SocialVideo015}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-015/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="lesson-ai-admin-00-video-00"
        component={LessonVideoAiAdmin00}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 60,
          endDuration: 10,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 60
          const endDuration = 10
          const response = await fetch(staticFile('video-ai-admin-00/text.json'))
          const captions = await response.json()
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-016"
        component={SocialVideo016}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-016/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

      <Composition
        id="social-017"
        component={SocialVideo017}
        durationInFrames={FRAME_RATE}
        fps={FRAME_RATE}
        width={FRAME_WIDTH}
        height={FRAME_HEIGHT}
        schema={LessonVideoPropsSchema}
        defaultProps={{
          captions: [],
          titleDuration: 0,
          endDuration: 3,
          allDuration: 100,
        }}
        calculateMetadata={async ({ props }) => {
          const titleDuration = 0
          const endDuration = 3
          const response = await fetch(staticFile('social-017/text.json'))
          const captions = await response.json()
          console.log(captions)
          const duration = Math.ceil(captions.at(-1).end * FRAME_RATE) + titleDuration + endDuration

          return {
            durationInFrames: duration + 3,
            props: {
              ...props,
              captions: transformCaptionsToFrames(captions),
              allDuration: duration,
            }
          }
        }}
      />

    </>
  )
}

const transformCaptionsToFrames = (captions: Captions): Captions => {
  return captions.map((caption) => {
    return {
      ...caption,
      start: Math.ceil(caption.start * FRAME_RATE),
      end: Math.ceil(caption.end * FRAME_RATE),
    }
  })
}

