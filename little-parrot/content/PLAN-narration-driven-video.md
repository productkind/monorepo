...
> **Status: built.** Phases 1-6 are done and three videos are migrated (social-017, social-016,
> social-000), each rendering from its definition with no hand-written durations. What is left is
> phase 0's dashboard step (rotate the key), migrating the remaining 29 videos with
> `npm run migrate`, and phase 7's cleanup. See "What was built" at the end.
...
- Migrate the remaining 29 videos with `npm run migrate`, deleting each component as it goes.
  `LessonVideo.tsx` and `Root.tsx` are down 333 lines from three videos; the rest follows.
- Phase 7: retire `content/audio/` once every video builds from cache.
