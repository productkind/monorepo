---
challenge: "8 - Build a Sustainable Business"
---
## Accountant Call Preparation Template

Use this template to prepare for your first conversation with an accountant. Fill in your details before the call so you can make the most of the time, and bring the questions below so you walk away with clear answers, not more confusion.

### Your details (fill in before the call)

Write these down so you can share them with the accountant at the start of the conversation. It helps them give you specific advice instead of generic answers.

- **Country / region:** [Where you live and where your customers are]
- **What you built:** [Brief description of your digital product]
- **Current stage:** [e.g., "I have 5 paying users" or "I haven't launched yet but I'm about to"]
- **Expected monthly revenue:** [Your best estimate, even if it's small, e.g., "$50/month"]
- **Monthly costs:** [e.g., "$8/month for domain and email"]
- **Day job:** [Do you have one? Are you employed or freelancing?]
- **Have you earned any income from this product yet?** [Yes/No, and roughly how much]

### Questions to ask

#### Getting started legally

- What's the simplest way to operate legally in my country? Do I need to register anything, or can I start earning without registration?
- At what point should I register a company? Is there an income threshold that triggers this?
- What's the difference between operating as a sole trader and setting up a company? What are the costs of each?
- Are there any registrations or notifications I need to make before I start accepting payments?

#### Tax obligations

- What taxes will I owe on income from a digital product? (Income tax, social security contributions, anything else?)
- At what income level do I need to register for VAT or sales tax?
- Do I need to charge VAT or sales tax on digital products? Does it matter where my customers are located?
- If my customers are in other countries, are there any international tax obligations I should know about?
- How often do I need to file taxes? Monthly, quarterly, annually?
- Are there any tax benefits or simplified schemes for people earning below a certain threshold?

#### Deductible expenses

- What business expenses can I deduct? (Domain, hosting, software tools, internet, home office?)
- Do I need receipts for everything? How should I store them?
- Can I deduct expenses I paid before I officially started the business?

#### Record-keeping

- What records do I need to keep from day one?
- Is a simple spreadsheet with income and expenses enough, or do I need accounting software?
- Do you recommend any specific tools for tracking income and expenses at this stage?
- How long do I need to keep financial records?

#### Planning ahead

- At what revenue level does my tax situation change significantly? (So I can plan for it)
- Is there anything I should set aside from each payment for taxes? What percentage would you recommend?
- When should I come back to talk to you again? What would trigger the need for another conversation?

### During the call

Take notes next to each question. Write down the specific answers for your country and situation. The accountant's answers will replace the generic advice in the course with advice that's tailored to you.

### After the call

- [ ] Write down the legal structure the accountant recommended (sole trader, company, other)
- [ ] Note the tax registration threshold for your country
- [ ] Note the VAT/sales tax threshold and whether it applies to digital products
- [ ] Set up your record-keeping system (even a simple spreadsheet)
- [ ] Set aside the recommended percentage from each payment for taxes
- [ ] Put a reminder in your calendar for when to revisit (e.g., when you hit a certain revenue level)
