...
The terms from the "AI Prompts for Product Managers" course in one place, with what each one changes about the way you prompt. Look something up here when you hit the word in a meeting, or when the AI does something odd and you want to know whether it's just behaving as expected.
...
| **Training data** | The text the model learned from: books, articles, websites, code, forums. "Large" (in large language models) refers to how much of it there was. | It has seen millions of LinkedIn posts, exec memos, and release announcements. Naming the format taps into all of that. |
...
| **System prompt** | Hidden instructions written by whoever built the tool, sent alongside your prompt every time. They set the ground rules for how the model behaves. | Explains why a model sometimes refuses things or defaults to a particular style. |
...
An LLM is an extremely well-read autocomplete. It has read more books, articles, and code than you ever will, and it's very good at producing the next plausible output. It has never seen your product, your customers, or your roadmap.

That's the whole trade. It brings the structure and the fluency. You bring the facts and the judgement.
