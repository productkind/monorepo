...
- `company_type` is a company-**size** bucket (e.g. `startup-1-50-employees`). The profiles table has no actual company _name_, so size is the closest field available.
