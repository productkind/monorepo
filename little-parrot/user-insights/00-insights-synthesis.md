...
**Application beats definition** (strong, cross-course). Exercises that ask learners to _produce_ something (write a real prompt, compare before/after, the 3-part Step 75 reflection) consistently pull richer thinking than open recall prompts ("What did you notice?" / "Define an LLM"), which yield one-word answers.
...
**The book-club example is too sticky.** On the product-breakdown step, 6+ learners submitted the book-club sample verbatim as their own answer, and in the Lovable course many pasted Dalmie's book-club prompt into the reflection box unchanged. The example is acting as an answer, not a model. Fix: show 2-3 varied examples, or have learners draft their own idea _before_ the sample appears.
