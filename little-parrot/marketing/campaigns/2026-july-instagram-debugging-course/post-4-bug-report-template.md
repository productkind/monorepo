...
| --- | --- | --- |
| 1 (cover) | Grey bubble "the button doesn't work :(" vs a tidy report card | "How you describe a bug decides how fast it gets fixed. The report template that gets it done in one round. 👇" |
| 2 | Template section card | "Start with the scene: WHERE you were ('on the /books page, logged in as a regular user') and WHAT you did ('clicked Vote next to Atomic Habits'). Lovable can't fix what it can't reproduce." |
...
The copy-paste version with a filled-in example comes with the full course, Fix Bugs with Confidence: Debugging Your Lovable App, on LittleParrot.app. First challenge free.
