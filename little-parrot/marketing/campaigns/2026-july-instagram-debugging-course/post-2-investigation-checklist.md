...
| --- | --- | --- |
| 1 (cover) | A magnifying glass over an app window | "Something broke. Before you ask for a fix, find one clue. Here's where developers look, in order. 👇" |
...
| 5 | Lovable Cloud logs and a database table | "4️⃣ Check backstage: in Lovable's Cloud tab, open Logs → Function logs for errors, then check the Database. Did the row you expected actually appear? Does it look right?" |
...
The difference: "The vote button doesn't work" invites a guess, while "POST votes returned 403" tells Lovable where the problem lives, and the fix usually lands on the first try.
