...
weeks.forEach((week) => {
  ...
  days.forEach(async (day) => {
    ...
    if (days.length > 1) {
      await combineSvgFiles(
        dailyFiles,
        path.join(distPath, 'strip.svg'),
        [cols, rows],
    ...
    dailyFiles.forEach(async (filePath) => {
      const outputPngPath = path.join(
        distPath,
        namePrefix + path.basename(filePath).replace('.svg', '.png'),
      )
      await rasterizeSvg({
    ...
    console.log(
      `Generated combined PNG for ${day} at ${path.join(distPath, namePrefix + 'strip.png')}`,
    )
  })
})
...
const combineSvgFiles = async (
  ...
  const combinedDoc = parser.parseFromString(
    `<svg xmlns="${svgNS}" width="${totalWidth}" height="${totalHeight}"></svg>`,
    'image/svg+xml',
  )
  ...
  inputPaths.forEach((filePath, index) => {
    ...
    const defs = Array.from(clonedSvg.childNodes).find(
      (child) => child.nodeType === 1 && child.nodeName === 'defs',
    )
    if (defs) {
    ...
    // Calculate position
    const isThirdAndLast =
      index === inputPaths.length - 1 && inputPaths.length % (cols * rows) === 3
    ...
    g.setAttribute(
      'transform',
      `translate(${x}, ${y}) scale(${scaleX}, ${scaleY}) translate(${translateX}, ${translateY})`,
    )

    // Copy all child elements except defs
    Array.from(clonedSvg.childNodes).forEach((child) => {
      if (child.nodeType === 1 && child.nodeName !== 'defs') {
        // Element node, not defs
