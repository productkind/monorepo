...
- **Little Parrot course content defines a new term the first time it appears**: "MVP (Minimum Viable Product)", "kebab-case (all lowercase, words separated by hyphens)". Teaching the real term is the goal, so never swap it for a dumbed-down paraphrase; inside a course the fix for a bare term is a definition, not a simpler word. This is a course rule only. In every other format (captions, carousel slides, LinkedIn posts, Substack articles, emails, toolkit items, landing page lines) an inline definition is not required and should not be asked for. Use the term plainly, or pick a word the reader already knows. A bracketed definition bolted onto a caption or a slide only makes the line longer.
...
- A **real technical term** used as the right word (static, dynamic, commit, prompt). Little Parrot course content defines it on first use; outside course content a missing inline definition is not a fault, so never prescribe one.
