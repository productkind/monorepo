...
- **Captions follow the carousel's teaser principle**: expand the tease in natural sentences, in the reader's search language, but hold the detail, the how, and the nuance back for the article or course the post drives to.
...
The critic focuses on language; this skill remains the source of truth for structure (hook-first openings, CTA type, hashtags). What the user sees is the **final captions plus a short summary** of what the critic flagged and how it was resolved, not every round, unless they ask to see the drafts.

## Caption rules shared by all platforms

- **The first line is the hook AND the search phrase.** Captions are keyword-indexed (Instagram in-app search plus Google for public professional accounts; TikTok search; the YouTube Shorts title). Open with the phrase a learner would actually type ("how to build an app without coding"), written as a natural spoken line, never tag-speak or keyword stuffing. For a hook, recognition beats explanation, you want the reader's first reaction to be "that's me," then relief.
...
- Write a long description: roughly 100-200 words (about 600-1,200 characters; the limit is 4,000). TikTok itself says "writing a long description can help get 3x more views on average", and the extra words give its search more to index. Only the first ~80-100 characters show before "more", so put the caption in this order: hook + exact search phrase and audience; several sentences of useful value or context that extend what the video shows, in the words people search for; payoff-specific follow ask; 3-5 niche hashtags. Do not pad the caption or paste in the full script.
...
- **The title is the hook**: 20-40 characters, search phrase first (short keyword-first titles perform best on trending Shorts). Include a women-specific keyword in the title whenever it fits ("AI for women...", "women building...").
...
- Hook + point inside the first ~140 characters (the "see more" fold). Length: whatever the idea needs up to ~2,500 characters; LinkedIn is the one platform where substantive length outperforms, but a short post that says one thing well beats a padded one.
- No hashtags.
