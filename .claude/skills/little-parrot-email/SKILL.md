...
- **Header logo: use `logo-little-parrot-email.png`**, with explicit `width="220" height="63"`. It is 660x189, which is **3x** the 220px display width (most of the audience is on 3x-density iPhones), rendered from `logo-little-parrot.svg` with a real alpha channel so it sits cleanly on the tinted gradient header. **Raster is the default for email images**, since it is the one format every client and security gateway renders; see the format note below.
  - **Never use `logo-little-parrot.png`.** Despite the name it is a _white-backed_ variant with zero transparent pixels and a different 3.15:1 ratio, so it paints a white block on the gradient header and gets the height wrong. Nothing should reference it.
  - **PNG over SVG is a default, not a fix for a known bug.** SVG does render in the Gmail iOS app: verified by Kinga in August 2026 on a Google account, which contradicts caniemail's `image-svg` entry reporting Gmail iOS/Android as partial ("only works with non Google accounts"). That data appears stale, most likely because Gmail proxies images through googleusercontent. Two lessons: **treat caniemail as a guide to what to test, not proof**, and **never swap a working asset without inspecting the replacement** (the white-backed PNG above shipped that way once).
  - The six transactional templates in `little-parrot-awakens/supabase/functions/` still use the SVG and have no `width`/`height` on the logo. That is a known inconsistency, left alone deliberately; changing live transactional email is its own task.
- **CSS gradients need a solid fallback.** Outlook (Word engine) ignores `linear-gradient`. Wherever a gradient carries meaning (e.g. an accent bar that would otherwise vanish), set a solid `background-color` _first_, then the gradient, so it degrades to a visible colour: `background-color: #fdd825; background: linear-gradient(180deg, #ffb65b, #fdd825, #fbfb00, #8efd23, #00ed70);`
...
>
  <tr>
    <td style="padding: 0 6px;">
      ...
      >
        <img
          src="https://littleparrot.app/icon-linkedin.png"
          width="24"
          height="24"
          alt="LinkedIn"
    ...
    <td style="padding: 0 6px;">
      ...
      >
        <img
          src="https://littleparrot.app/icon-instagram.png"
          width="24"
          height="24"
          alt="Instagram"
          style="vertical-align: middle; border: 0;"
    ...
    <td style="padding: 0 6px;">
      <a
        href="https://www.tiktok.com/@littleparrot.app"
        style="text-decoration: none; font-size: 12px;"
      >
        <img
          src="https://littleparrot.app/icon-tiktok.png"
          width="24"
          height="24"
          alt="TikTok"
          style="vertical-align: middle; border: 0;"
    ...
    <td style="padding: 0 6px;">
      <a
        href="https://www.youtube.com/@productkind"
        style="text-decoration: none; font-size: 12px;"
      >
        <img
          src="https://littleparrot.app/icon-youtube.png"
          width="24"
          height="24"
          alt="YouTube"
          style="vertical-align: middle; border: 0;"
...
- **Short sentences, mostly one idea each, and active voice, because the email gets about nine seconds** (see _Built to be skimmed_). This is a scanning rule, not a comprehension one, so a longer sentence is fine where it genuinely reads better. Cut a first draft by about half.
...
- **"Concrete over vague" applies to our facts, not to the reader's life.** Being specific means not hiding a _product_ fact behind a fuzzy word ("takes about 10 minutes" beats "quick"). It does not license asserting a fact about the reader's week, workload or circumstances. "Start with whichever one matches what you're working on this week" presumes she has a live project, and on a list that includes free-plan and lapsed readers that tells anyone between bursts of work that the button isn't for her yet. "Start with whichever one you need first" keeps the nudge and drops the assumption. Watch for this hardest on the line right before a CTA, whose job is to lower the barrier.
- **Name the source of every factual-sounding claim before it ships, especially your own.** A sentence that reads like observed detail invites less scrutiny, not more, which is exactly backwards. In the July draft "A typo or a colour change costs you Lovable credits, the same as a real feature" survived two critic rounds and was praised as the strongest line, because it was specific and well turned. It was invented. The course's own `*-tool-facts.md` verified something narrower (the cost attaches to _asking Lovable's AI_, not to the change) and carried an explicit writer instruction the line broke: Visual Edits already changes copy free, so never claim local editing is the only free route. Before sending, for each claim ask: which file, page or research note says this? If the answer is "it sounds right", it is not sourced. **Third-party pricing and free tiers drift**, so re-check them against the facts sheet's own freshness window (they carry a "verify again after ~2 weeks" line) rather than trusting a sheet written weeks earlier.
- **Don't carry a phrase from course content into email copy without re-reading what it now does.** A phrase can be neutral as a scenario label a learner scans and loaded as an assertion about the reader. A toolkit table row reading "Chasing an urgent fix without nagging" became "chase a status update without sounding like you're nagging" in a draft of the July update: "nagging" is a stereotype aimed at women, and putting it in outcome copy told a list of women that's how they sound. State the payoff instead ("Ask for an urgent fix and get a reply"). Persona research quoting a user's own feeling ("I feel like a nag") is a different thing and is fine.
...
- **Check actions are logically consistent.** Match the verb to what the button does — "add to your profile" is a silent credential, so a "tag us / we'd love to cheer you on" ask belongs with _sharing a post_, not with adding a credential.
