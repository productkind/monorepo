...
export const createFileOperations = (fileSystem: FileSystemService): FileOperations => {
  const transformFile: FileOperations['transformFile'] = ({
    input,
    output,
    readError,
    writeError,
  }: Options): GetTransformSet<string, string> => {
    const readFile = () =>
      fileSystem
        .readFile(input, 'utf-8')
        .pipe(
          catchAndRethrow(
            (cause) => new Error(readError?.(input) ?? `Could not read input: ${input}`, { cause }),
          ),
        )
    const writeFile = (content: string) =>
      fileSystem
        ...
        .pipe(
          catchAndRethrow(
            (cause) =>
              new Error(writeError?.(output) ?? `Could not write output: ${output}`, { cause }),
          ),
        )
    return createGetTransformSet(readFile, writeFile)
  }

  const transformFileContext: FileOperations['transformFileContext'] = <CONTEXT>({
    input,
    output,
    readError,
    writeError,
  ...
  const copyFile: FileOperations['copyFile'] = (
    source: string,
    destination: string,
  ): Observable<void> => {
  ...
  const copyDirectory: FileOperations['copyDirectory'] = (
    source: string,
    destination: string,
    exclude: string[] = [],
  ): Observable<void> => {
    const excludeGlobs = exclude.map((pattern) => fileSystem.glob(path.join(source, pattern)))
    return forkJoin([fileSystem.readDirDeep(source), ...excludeGlobs] as const).pipe(
      ...
      mergeMap((filePairs) => forkJoin(filePairs.map(({ from, to }) => copyFile(from, to)))),
      map(() => undefined),
    )
  }
