...
export const createTranspilerService = (fileSystem: FileSystemService): Transpiler => {
  const transpileDirAsync = async ({
    ...
    program.emit(undefined, writeFile, undefined, undefined, { after: [rewriteImportsTransformer] })
    ...
    for (const inputFile of tsFiles) {
      ...
      if (sourceFile) {
        ...
        if (syntacticDiagnostics.length > 0) {
          const formatted = ts.formatDiagnosticsWithColorAndContext(syntacticDiagnostics, {
