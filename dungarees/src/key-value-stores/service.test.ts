...
test('KeyValueStore rejects assigning store with wrong keys at the type level', () => {
  const rawStore = createMemoryRawKeyValueStore()
  const store = createKeyValueStore(rawStore, {
    PATH2: z.string(),
    OTHER: z.string(),
  })
  store.set('PATH2', '/usr/bin')
  store.set('OTHER', 'value')
  // @ts-expect-error store does not have PATH key
  const narrower: KeyValueStore<{ PATH: z.ZodString }> = store
  expect(() => narrower.get('PATH')).toThrow('Invalid key: "PATH"')
})

test('KeyValueStore with more keys is assignable to a type expecting fewer keys', () => {
  const rawStore = createMemoryRawKeyValueStore()
  const store = createKeyValueStore(rawStore, {
    PATH: z.string(),
    OTHER: z.string(),
  ...
  store.set('OTHER', 'value')
  const narrower: KeyValueStore<{ PATH: z.ZodString }> = store
  const value: string = narrower.get('PATH')
