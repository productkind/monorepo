...
test('subProcessOperations.isExecutableFile is false for user-executable file when called by non-owner', async () => {
  const fileSystem = createFakeFileSystem({
    '/exec.sh': '#!/bin/sh\necho hello',
  })
...
test('subProcessOperations.isExecutableFile is true for user-executable file when called by owner', async () => {
  const originalGetuid = (process as unknown as { getuid?: (() => number) | undefined }).getuid
  const originalGetgid = (process as unknown as { getgid?: (() => number) | undefined }).getgid
...
test('subProcessOperations.isExecutable is true for a command found in PATH', async () => {
  const fileSystem = createFakeFileSystem({
    '/usr/bin/npm': '#!/bin/sh',
  })
...
test('subProcessOperations.isExecutable is false for a command not found in PATH', async () => {
  const fileSystem = createFakeFileSystem({
    '/usr/bin/npm': '#!/bin/sh',
  })
  fileSystem.chmodSync('/usr/bin/npm', ALL_EXECUTE)
...
test('subProcessOperations.isExecutable is false for a command in PATH that is not executable', async () => {
  const fileSystem = createFakeFileSystem({
    '/usr/bin/npm': '#!/bin/sh',
  })
...
test('subProcessOperations.isExecutable delegates to isExecutableFile for paths with /', async () => {
  const fileSystem = createFakeFileSystem({
    '/usr/bin/npm': '#!/bin/sh',
  })
  fileSystem.chmodSync('/usr/bin/npm', ALL_EXECUTE)
...
test('subProcessOperations.isExecutable finds command in second PATH directory', async () => {
  const fileSystem = createFakeFileSystem({
...
test('subProcessOperations.runValidated runs command when executable and cwd accessible', async () => {
  const fileSystem = createFakeFileSystem({
    '/usr/bin/ls': '#!/bin/sh',
  })
...
test('subProcessOperations.runValidated returns error when cwd is not accessible', async () => {
  const fileSystem = createFakeFileSystem({
    '/usr/bin/ls': '#!/bin/sh',
  })
  fileSystem.chmodSync('/usr/bin/ls', ALL_EXECUTE)
...
test('subProcessOperations.runValidated runs without cwd validation when no cwd provided', async () => {
  const fileSystem = createFakeFileSystem({
    '/usr/bin/ls': '#!/bin/sh',
  })
  fileSystem.chmodSync('/usr/bin/ls', ALL_EXECUTE)
...
test('subProcessOperations.runSilentUntilErrorValidated returns no output on success', async () => {
  const fileSystem = createFakeFileSystem({
    '/usr/bin/ls': '#!/bin/sh',
  })
  fileSystem.chmodSync('/usr/bin/ls', ALL_EXECUTE)
...
test('subProcessOperations.runSilentUntilErrorValidated returns stderr error for inaccessible cwd', async () => {
  const fileSystem = createFakeFileSystem({
    '/usr/bin/ls': '#!/bin/sh',
  })
  fileSystem.chmodSync('/usr/bin/ls', ALL_EXECUTE)
...
test('subProcessOperations.runSilentUntilErrorValidated shows all output on process stderr', async () => {
  const fileSystem = createFakeFileSystem({
    '/usr/bin/ls': '#!/bin/sh',
  })
  fileSystem.chmodSync('/usr/bin/ls', ALL_EXECUTE)
