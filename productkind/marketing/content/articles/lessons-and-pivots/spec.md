---
status: posted
channels: [linkedin, instagram, tiktok]
account: kinga, productkind
---

# Carousel: What a year of flops taught me

**Source article:** `thoughts/assets/articles/021-what-we-learned-from-building-digital-products-lessons-and-pivots.md` **Channels:** Kinga's personal LinkedIn, Instagram, TikTok **Date:** July 2026

**Story arc:** the honest tiny number I opened with (slide 1), that I wrote about it anyway (2), what didn't work (3), the turn: each flop was telling me something (4), the two lessons the flops gave me (5 to 6), the pivot they pointed to (7), the invitation (8). Follows the narrative of Kinga's own LinkedIn post about the article. A fellow-learner retrospective, never from above; the throughline is the honest telling, not an added moral.

## Format

**Carousel, 8 slides, 1080 x 1350 (4:5 portrait).** Editorial, personal productkind style: warm palette, generous whitespace, Kim mascot on the cover and CTA. A big-number treatment for the Instagram stats, a black statement card for the two hard-won lessons, a warm gradient for the pivot.

**Export note:** the same 1080 x 1350 export works on all three channels. TikTok photo mode pads the sides; keep all text inside the central safe area.

**Full-fidelity rule:** every word that appears on a slide is in this table, verbatim. The implemented carousel may not add copy; `check.py` in `productkind/carousel-design/` enforces it.

| Slide | Visual | Text on slide (verbatim) |
| --- | --- | --- |
| 1 (cover) | Brand cover with Kim mascot; calm, honest. Big numbers do the work | Kicker: "MY FIRST FULL YEAR OF RUNNING MY BUSINESS." Runner: "LESSONS AND PIVOTS", new line: "SWIPE →" |
| 2 | Simple centred copy card, personal | Headline: "We posted 76 times on Instagram, and went from 0 followers to 33." Body: "That's a tiny number. I wrote about it anyway, along with everything else I tried this year." |
| 3 | Copy + black statement card | "Some of it really didn't work. The comics took hours to make." Black card: "And never reached anyone new on Instagram." |
| 4 | Big centred statement, the hinge of the story | "What helped most was working out what each flop was telling me." |
| 5 | Instagram carousel mockup in a frame + copy | "We learned Instagram carousels don't get shown to non-followers. A poor way to get discovered there." |
| 6 | Photo/illustration frame of a talk + copy | "A conference full of the wrong audience taught me to work out who I'm building for before I turn up, not after." |
| 7 | Warm gradient statement card, the payoff | "So every experiment pointed somewhere better. Including the pivot: helping women who can't code yet build their own products." |
| 8 (CTA) | Brand card: Thoughts by productkind logo/Kim, URL badge, offer line, share-ask | "The pivot, and the flops that led to it, is what the full article is about." Badge: "PRODUCTKIND.SUBSTACK.COM" · "Full article on our Substack 💜" · "Got a project your doubts keep talking you out of? Tell me in the comments." |

**Alt text (for the post):** A personal carousel about a first full year building digital products: 76 Instagram posts that reached 33 followers, comics that took hours and never found new people, and the lessons each flop gave, that Instagram carousels don't reach non-followers, and to know who you're building for before a conference, leading to a pivot towards helping women who can't code yet build their own products.

## LinkedIn caption (exact)

We posted 76 times on Instagram over 19 weeks. We started at 0 followers and got to 33.

That's a tiny number, and I wrote about it anyway, along with everything else from my first full year building digital products in my company.

To be honest, some of it really didn't work. Those comics took hours to make and never reached anyone new on Instagram. What helped most was working out what each flop was telling me. The Instagram one taught me that carousels don't get shown to non-followers, so they're a poor way to get discovered. A conference full of the wrong audience taught me to work out who I'm building for before I turn up, not after.

So every one of those experiments pointed me somewhere better, including the pivot to helping women who can't code yet build their own products.

That pivot, and the flops that led to it, is what the article is about. If there's a project your own doubts keep talking you out of, I'd love to know what it is in the comments.

(Article link in the first comment.)

## Instagram caption (exact)

We posted 76 times on Instagram over 19 weeks. We started at 0 followers and got to 33.

That's a tiny number, and I wrote about it anyway, along with everything else from my first full year building digital products.

Some of it really didn't work. Those comics took hours to make and never reached anyone new here. What helped most was working out what each flop was telling me. Instagram taught me that carousels don't get shown to non-followers, so they're a poor way to get discovered. A conference full of the wrong audience taught me to work out who I'm building for before I turn up, not after.

So every experiment pointed me somewhere better, including the pivot to helping women who can't code yet build their own products. The full story is in my newsletter, link in bio. 💛

If there's a project your own doubts keep talking you out of, I'd love to hear it in the comments.

. . . #buildinpublic #vibecoding #aiforwomen #womenintech #indiehacker #digitalproducts #solopreneur #startingfromscratch #femalefounders #learninpublic

## TikTok caption (exact)

76 Instagram posts, 33 followers, and a year of flops that each taught me something, including the pivot to helping women who can't code yet build their own products. Full story in my newsletter, link in bio. 💛

_(House style keeps hashtags to Instagram; if you want discovery tags on TikTok, reuse two or three from the Instagram set, e.g. #buildinpublic #vibecoding #womenintech.)_

## Posting notes

- LinkedIn: post the carousel as a PDF document post; put the Substack article link in the first comment, as the caption promises.
- Instagram: update the link in bio to the article before posting.
- TikTok: photo mode carousel; same slide exports.
- Slide 1 numbers source: the article ("What Didn't Work", section 1). Slide 5 lesson: same section. Slide 6 lesson: "Giving Talks and Running Workshops at Conferences".
